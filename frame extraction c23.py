...
def extract_frames_for_xception(base_video_path, output_base_path, frames_per_video=30):
    ...
    for cat in categories:
        ...
        print(f"\n{'=' * 70}")
        print(f"MULAI EKSTRAKSI FRAME: {cat.upper()} ({num_videos} Video)")
        print(f"{'=' * 70}")
        ...
        for i, video_path in enumerate(video_files, 1):
            ...
            if extracted_count > 0:
                ...
                print(f"[{i}/{num_videos}] FAILED: {video_name} (Tidak ada frame terbaca)")

    return total_extracted_all

current_dir = os.path.dirname(os.path.abspath(__file__))
video_path_c23 = os.path.join(current_dir, 'dataset', 'FaceForensics++_C23')
frames_output_path = os.path.join(current_dir, 'dataset', 'frames_C23')

if __name__ == "__main__":
    if os.path.exists(video_path_c23):
        total = extract_frames_for_xception(video_path_c23, frames_output_path)
        print(f"EKSTRAKSI SELESAI!")
        print(f"Total frame baru yang disimpan: {total}")
        print(f"Lokasi: {os.path.abspath(frames_output_path)}")

    else:
        print(f"Error: Folder dataset tidak ditemukan di {video_path_c23}")